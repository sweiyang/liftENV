...
Q = np.zeros(shape=(pow(2, 2*13), ACTION_SPACE_n), dtype=float)
episode_reward = 0
...
print('max episode:', NUM_EPISODES, 'NUM LIFTS:', NUM_LIFTS, "NUMFLOOR:", NUM_FLOORS, "MAXPERSON", MAX_PERSON)
for e in range(NUM_EPISODES):
    ...
    while timestepHours != 24:
        ...
        # check whether s_prime is in Q_s


        # updating Q table
        Q[s_index][action] = Q[s_index][action] + learning_rate * (
                r + discount * np.argmax(Q[sprime_index]) - Q[s_index][action])
        # print(s_index)
        # print(sprime_index)
        # print(action)
...
np.savetxt("data.csv", array_episode_reward, delimiter=",")
